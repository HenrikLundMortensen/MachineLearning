...
class LogisticClassifierTwoClass(MlModel):
    ...
    def train(self, X_, y, reg=1e-4, lr=0.1, epochs=5, batch_size=16):
        ...
        ### YOUR CODE HERE
        self.w = log_reg.mini_batch_grad_descent(X, y,reg=reg, lr=lr, batch_size=batch_size, epochs=epochs)
        ### END CODE
    ...
    def predict(self, X_):
        ...
        ### YOUR CODE HERE
        predictions = np.round(log_reg.logistic(self.w.dot(X.T)))
        ### END CODE
    ...
    def probability(self, X_):
        ...
        ### YOUR CODE HERE
        log_reg.logistic(self.w.dot(X.T))
        ### END CODE
...
class LogisticClassifier(MlModel):
    ...
    def train(self, X, y, reg=1e-4, lr=0.1, epochs=5, batch_size=16):
        ...
        for c in self.classes:
            yc = np.zeros(y.shape)
            yc[np.where(y==c)] = 1
            LCc = LogisticClassifierTwoClass()
            LCc.train(X,yc)
            self.models.append(LCc)
        ### END CODE
...
def model_accuracy(model, X, y):
    ...
    acc = np.mean([yhat[i]==y[i] for i in range(len(y))])
    ### END CODE
    return acc
