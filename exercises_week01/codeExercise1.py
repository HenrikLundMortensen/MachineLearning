...
def standardize_strings(string_list):
    ...
    Standardize strings by 
        ...
        2. remove any non-alphabetic characters, i.e. 
           characters not in [a-zÃ¦Ã¸Ã¥]. (example "l33t_h@x" -> "lthx")
    ...
    You are promised that the only characters in the input that are not [a-zÃ¦Ã¸Ã¥] 
    ...
    # res = [ string.replace(char,'') for char in badlist for string in string_list]
    for string in string_list:
        for char in badlist:
            string = string.replace(char,'')
        res.append(str.lower(string))
    ### END CODE
...
def split_strings(strings):
    ...
    ### YOUR CODE
    word_lists = list(string.split(' ') for string in strings)
    ### END CODE
...
def remove_irrelevant_words(word_lists, bad_words=set()):
    ...
    ### YOUR CODE, 1-3 lines
    pruned_word_lists = [[word for word in word_list if word not in bad_words] for word_list in word_lists]
    ### END CODE
...
class TextClassifier():
    ...
    def  __init__(self):
        ...
        self.stopwords = {'over', 'hvad', 'man', 'sit', 'ogsÃ¥', 'han', 'eller', 'blive', 'denne', 'det', 'meget', 'til',
                          ...
                          'ad', 'pÃ¥', 'have', 'disse', 'for', 'skal', 'et', 'bliver', 'min', 'sig', 'i', 'nu', 'og', 'har', 'mine', 'vi',
                          'da', 'selv', 'jer', 'mit', 'sÃ¥dan', 'hans', 'skulle', 'noget', 'som', 'vor', 'hendes', 'om', 'kunne', 'nogle',
                          'med', 'vÃ¦re', 'vÃ¦ret', 'de', 'sine', 'men', 'dette', 'sin', 'deres', 'jo', 'ud', 'hun', 'er', 'ned', 'mange',
                          'mod', 'dog', 'nÃ¥r', 'end', 'blev', 'dem', 'formÃ¥l', 'selskabets', 'virksomhed', 'samt'}
...
# You should not waste time reading the test code. Just 
...
def test_vectorize():
    ...
    target[2, 0] = 1
    target[2, 10] = 1
    assert np.allclose(string_vectors, string_vectors)
...
# Selskabets formÃ¥l er formueadministration     
# Selskabets formÃ¥l er at udÃ¸ve virksomhed med handel og service samt aktiviteter i tilknytning hertil. 
